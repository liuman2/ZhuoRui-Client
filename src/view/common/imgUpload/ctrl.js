require('libs/jcrop/js/jquery.Jcrop');

var httpHelper = require('js/utils/httpHelper');

module.exports = function($scope, $state, $element) {
  // container
  var $ele = $($element);

  // router
  $scope.parent_sref = $state.current.name.replace(/.imgupload/g, '');
  ...
  new Cut($ele.find("#cover"), options, $ele.find('.img-select'), $ele.find('.img-upload'))

  // button event
  $scope.onConfirm = function() {
    if (!$scope.hasDate) return false;

    $scope.$emit('imageUploadSuccess', id, url);
    $state.go('^');
  }
};
